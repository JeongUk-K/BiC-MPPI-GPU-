...
// are excluded in the same way as the former DBSCAN implementations.
inline void runGPUKMeans(std::vector<std::vector<int>> &clusters,
  ...
  for (int i = 0; i < sample_count; ++i) {
    ...
    if (std::isfinite(costs(i)) && costs(i) < kCollisionCost &&
        finite_feature)
  ...
  const int k = std::max(1, std::min(param.clusters,
  ...
  // Deterministic farthest-point initialization. Round-robin labels make all
  // initial centroids converge near the global mean for shuffled MPPI samples,
  // which caused dozens of unnecessary Lloyd iterations.
  ...
  // Reuse device capacities across closed-loop solver iterations. CUDA malloc
  // per K-means call was a significant fixed cost, especially for smoke runs.
