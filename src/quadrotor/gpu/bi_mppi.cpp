...
int main(int argc, char **argv) {
  ...
  std::ofstream progress_csv("result_quadrotor_bi_mppi_progress.csv");
  csv << std::flush;
  ...
  for (int map = config.map_begin;
    ...
    initQuadrotorGpuVisLogger(vis_logger, config.save_rollouts, "bi_mppi",
    ...
    for (i = 0; i < maxiter; ++i) {
      ...
      solver.move();
      writeQuadrotorPathRow(path_csv, "bi_mppi", map, i + 1,
                            solver.x_init);
