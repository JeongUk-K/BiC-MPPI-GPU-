...
int main(int argc, char **argv) {
  ...
  std::ofstream path_csv("path_quadrotor_cluster_mppi.csv");
  writeQuadrotorPathHeader(path_csv);
  std::ofstream progress_csv("result_quadrotor_cluster_mppi_progress.csv");
  csv << std::flush;
  ...
  for (int map = config.map_begin;
    ...
    initQuadrotorGpuVisLogger(vis_logger, config.save_rollouts,
                              "cluster_mppi", map, model.dim_x, param.T,
                              collision_checker, param.x_init,
                              param.x_target);
    solver.setVisLogger(&vis_logger);

    writeQuadrotorPathRow(path_csv, "cluster_mppi", map, 0, solver.x_init);
    ...
    for (i = 0; i < maxiter; ++i) {
      ...
      solver.move();
      writeQuadrotorPathRow(path_csv, "cluster_mppi", map, i + 1,
                            solver.x_init);
