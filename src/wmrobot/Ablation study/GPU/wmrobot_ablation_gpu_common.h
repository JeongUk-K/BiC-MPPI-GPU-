...
inline double gpuQuantile(std::vector<double> values, double q) {
  if (values.empty()) {
...
inline void writeGpuSummaryRow(std::ofstream &csv,
  ...
  csv << summary.variant << ',' << gpuCsvDouble(summary.success_rate) << ','
      ...
      << summary.num_success << ',' << summary.variant << ',' << summary.num_runs
      << ',' << summary.num_success << ',' << summary.nfail << ','
      << gpuCsvDouble(summary.success_rate) << ','
      << gpuCsvDouble(summary.avg_iter_success) << ','
      << gpuCsvDouble(summary.avg_total_elapsed_success) << ','
      << gpuCsvDouble(summary.avg_rollout_time_success) << ','
      << gpuCsvDouble(summary.avg_clustering_time_success) << ','
      << gpuCsvDouble(summary.avg_connection_time_success) << ','
      << gpuCsvDouble(summary.avg_guide_time_success) << ','
      << gpuCsvDouble(summary.total_elapsed_success_q1) << ','
      << gpuCsvDouble(summary.total_elapsed_success_q2) << ','
      << gpuCsvDouble(summary.total_elapsed_success_q3) << ','
      << gpuCsvDouble(summary.rollout_time_success_q1) << ','
      << gpuCsvDouble(summary.rollout_time_success_q2) << ','
      << gpuCsvDouble(summary.rollout_time_success_q3) << ','
      << gpuCsvDouble(summary.clustering_time_success_q1) << ','
      << gpuCsvDouble(summary.clustering_time_success_q2) << ','
      << gpuCsvDouble(summary.clustering_time_success_q3) << ','
      << gpuCsvDouble(summary.connection_time_success_q1) << ','
      << gpuCsvDouble(summary.connection_time_success_q2) << ','
      << gpuCsvDouble(summary.connection_time_success_q3) << ','
      << gpuCsvDouble(summary.guide_time_success_q1) << ','
      << gpuCsvDouble(summary.guide_time_success_q2) << ','
      << gpuCsvDouble(summary.guide_time_success_q3) << '\n';
}
...
summarizeGpuRuns(const std::string &variant,
  ...
  if (summary.num_success > 0) {
    ...
    summary.total_elapsed_success_q1 = gpuQuantile(success_elapsed_values, 0.25);
    summary.total_elapsed_success_q2 = gpuQuantile(success_elapsed_values, 0.50);
    summary.total_elapsed_success_q3 = gpuQuantile(success_elapsed_values, 0.75);
    summary.rollout_time_success_q1 = gpuQuantile(success_rollout_values, 0.25);
    summary.rollout_time_success_q2 = gpuQuantile(success_rollout_values, 0.50);
    summary.rollout_time_success_q3 = gpuQuantile(success_rollout_values, 0.75);
    summary.clustering_time_success_q1 =
        gpuQuantile(success_clustering_values, 0.25);
    summary.clustering_time_success_q2 =
        gpuQuantile(success_clustering_values, 0.50);
    summary.clustering_time_success_q3 =
        gpuQuantile(success_clustering_values, 0.75);
    summary.connection_time_success_q1 =
        gpuQuantile(success_connection_values, 0.25);
    summary.connection_time_success_q2 =
        gpuQuantile(success_connection_values, 0.50);
    summary.connection_time_success_q3 =
        gpuQuantile(success_connection_values, 0.75);
    summary.guide_time_success_q1 = gpuQuantile(success_guide_values, 0.25);
    summary.guide_time_success_q2 = gpuQuantile(success_guide_values, 0.50);
    summary.guide_time_success_q3 = gpuQuantile(success_guide_values, 0.75);
  }
  return summary;
}
