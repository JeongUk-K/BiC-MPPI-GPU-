...
struct BiMppiConfig {
  ...
  std::string result_prefix = "result_bi_mppi_se2";
...
void printUsage(const char *argv0) {
  std::cerr << "Usage: " << argv0 << " [options]\n"
            ...
            << "  --se2-weight-xy W    SE(2) xy weight. Default: 0.2\n"
            << "  --se2-weight-theta W SE(2) theta weight. Default: 1.0\n"
            << "  --result-prefix NAME CSV output prefix. Default: result_bi_mppi_se2\n"
...
BiMppiConfig parseArgs(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    } else if (key == "--result-prefix") {
      config.result_prefix = require_value(key);
...
int main(int argc, char **argv) {
  ...
  const std::string variant =
      (config.connection_metric == "se2") ? "BiC-MPPI-SE2" : "BiC-MPPI";
  std::vector<WmrobotGpuRunResult> runs;

  std::ofstream csv(config.result_prefix + ".csv");
  writeWmrobotGpuRunHeader(csv);
  csv.flush();

  std::ofstream progress_csv(config.result_prefix + "_progress.csv");
  ...
  for (int s = 0; s < config.start_cases; ++s) {
    ...
    for (int map = config.map_begin;
      ...
      solver.init(param);
      if (config.connection_metric == "se2") {
      ...
      const std::string vis_prefix =
          (config.connection_metric == "se2") ? "bi_mppi_se2" : "bi_mppi";
  ...
  std::ofstream summary_csv(config.result_prefix + "_summary.csv");
