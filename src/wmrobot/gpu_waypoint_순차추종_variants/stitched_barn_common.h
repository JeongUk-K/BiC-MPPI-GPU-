...
// Solver-specific parameters are in makeMppiConfig(), makeLogMppiConfig(),
...
inline Config makeBiMppiConfig() {
  ...
  // BiC-MPPI connects all remaining waypoint segments, then runs a global guide.
  cfg.bic_Tf = 50;
  cfg.bic_Tb = 50;
  cfg.Nf = 3000;
  cfg.Nb = 3000;
...
struct StepTiming {
  ...
  double cumulative_rollout = 0.0;
  double cumulative_clustering = 0.0;
  double cumulative_connection = 0.0;
  double cumulative_guide = 0.0;
...
inline void printUsage(const char *argv0) {
  std::cerr << "Usage: " << argv0 << " [options]\n"
            ...
            << "  --global-T N              Per-waypoint horizon for MPPI/Log/Cluster\n"
...
inline double stateOrNaN(const Eigen::VectorXd &state, int index) {
...
inline void writeTimingHeader(std::ofstream &csv) {
  csv << "scenario,solver,step,target_waypoint,reached_waypoints,"
         "total_waypoints,elapsed,elapsed_rollout,elapsed_clustering,"
         "elapsed_connection,elapsed_guide,cumulative_elapsed,"
         "cumulative_rollout,cumulative_clustering,cumulative_connection,"
         "cumulative_guide,x,y,theta\n";
}

inline void writeTimingRows(std::ofstream &csv, const std::string &solver_key,
                            int scenario_index, const RunStats &stats) {
  for (const auto &timing : stats.timings) {
    csv << scenario_index << ',' << csvString(solver_key) << ','
        << timing.step << ',' << timing.target_waypoint << ','
        << timing.reached_waypoints << ',' << timing.total_waypoints << ','
        << csvDouble(timing.elapsed) << ','
        << csvDouble(timing.elapsed_rollout) << ','
        << csvDouble(timing.elapsed_clustering) << ','
        << csvDouble(timing.elapsed_connection) << ','
        << csvDouble(timing.elapsed_guide) << ','
        << csvDouble(timing.cumulative_elapsed) << ','
        << csvDouble(timing.cumulative_rollout) << ','
        << csvDouble(timing.cumulative_clustering) << ','
        << csvDouble(timing.cumulative_connection) << ','
        << csvDouble(timing.cumulative_guide) << ','
        << csvDouble(stateOrNaN(timing.state, 0)) << ','
        << csvDouble(stateOrNaN(timing.state, 1)) << ','
        << csvDouble(stateOrNaN(timing.state, 2)) << '\n';
  }
}
...
inline void appendStepTiming(RunStats &stats, int step, int target_waypoint,
                             int reached_waypoints, int total_waypoints,
                             const Solver &solver) {
  StepTiming timing;
  timing.step = step;
  timing.target_waypoint = target_waypoint;
  timing.reached_waypoints = reached_waypoints;
  timing.total_waypoints = total_waypoints;
  timing.elapsed = solver.elapsed;
  timing.elapsed_rollout = solver.elapsed_rollout;
  timing.elapsed_clustering = solver.elapsed_clustering;
  timing.elapsed_connection = solver.elapsed_connection;
  timing.elapsed_guide = solver.elapsed_guide;
  timing.cumulative_elapsed = stats.elapsed;
  timing.cumulative_rollout = stats.elapsed_rollout;
  timing.cumulative_clustering = stats.elapsed_clustering;
  timing.cumulative_connection = stats.elapsed_connection;
  timing.cumulative_guide = stats.elapsed_guide;
  timing.state = solver.x_init;
  stats.timings.push_back(std::move(timing));
}

class WaypointBatchBiMPPI_GPU : public BiMPPI_GPU {
public:
  template <typename ModelClass>
  explicit WaypointBatchBiMPPI_GPU(ModelClass model) : BiMPPI_GPU(model) {}

  void solveWaypointGraph(const std::vector<Eigen::VectorXd> &anchors) {
    if (anchors.size() < 2) {
      throw std::runtime_error("At least two waypoint anchors are required");
    }
    ...
    const Eigen::VectorXd global_target = anchors.back();
    std::vector<Eigen::MatrixXd> segment_controls;
    std::vector<Eigen::MatrixXd> segment_paths;
    std::vector<Eigen::MatrixXd> forward_cluster_paths;
    std::vector<Eigen::MatrixXd> backward_cluster_paths;
    segment_controls.reserve(anchors.size() - 1);
    segment_paths.reserve(anchors.size() - 1);

    for (int segment = 0; segment + 1 < static_cast<int>(anchors.size());
         ++segment) {
      x_init = anchors[segment];
      x_target = anchors[segment + 1];
      ...
      for (int ci = 0; ci < static_cast<int>(clusters_f.size()); ++ci) {
        forward_cluster_paths.push_back(
            Xf.block(ci * dim_x, 0, dim_x, Tf + 1));
      }
      for (int ci = 0; ci < static_cast<int>(clusters_b.size()); ++ci) {
        backward_cluster_paths.push_back(
            Xb.block(ci * dim_x, 0, dim_x, Tb + 1));
      }
      ...
      const int best = bestConnectedReferenceIndex();
      if (best < 0) {
        throw std::runtime_error("BiC waypoint segment produced no connection");
      }
      segment_controls.push_back(Uc[best]);
      segment_paths.push_back(Xc[best]);
    }

    x_init = global_start;
    x_target = global_target;
    Eigen::MatrixXd Uref;
    Eigen::MatrixXd Xref;
    concatenateSegments(segment_controls, segment_paths, Uref, Xref);
    ...
    if (vis_logger && vis_logger->enabled) {
      ...
      if (!backward_cluster_paths.empty()) {
        vis_logger->saveTrajectories("backward_clusters",
                                     backward_cluster_paths);
      }
      if (!Xr.empty()) {
...
private:
  int bestConnectedReferenceIndex() const {
    double best_cost = std::numeric_limits<double>::max();
    int best_index = -1;
    for (int i = 0; i < static_cast<int>(Xc.size()); ++i) {
      bool feasible = true;
      double cost = 0.0;
      for (int t = 0; t < Xc[i].cols(); ++t) {
        if (collision_checker && collision_checker->getCollisionGrid(Xc[i].col(t))) {
          feasible = false;
          break;
        }
        cost += p(Xc[i].col(t), x_target);
      }
    ...
    if (best_index >= 0 || Xc.empty()) {
      return best_index;
    }
    return 0;
  }

  void concatenateSegments(const std::vector<Eigen::MatrixXd> &controls,
                           const std::vector<Eigen::MatrixXd> &paths,
                           Eigen::MatrixXd &Uref,
                           Eigen::MatrixXd &Xref) const {
    int total_steps = 0;
    for (const auto &U : controls) {
      total_steps += static_cast<int>(U.cols());
    }
    if (total_steps <= 0 || controls.size() != paths.size()) {
      throw std::runtime_error("Invalid BiC waypoint reference");
    }

    Uref = Eigen::MatrixXd::Zero(dim_u, total_steps);
    Xref = Eigen::MatrixXd::Zero(dim_x, total_steps + 1);
    Xref.col(0) = paths.front().col(0);

    int offset = 0;
    for (int i = 0; i < static_cast<int>(controls.size()); ++i) {
      const int Tr = static_cast<int>(controls[i].cols());
      if (paths[i].cols() != Tr + 1) {
        throw std::runtime_error("BiC segment path/control length mismatch");
      }
      Uref.middleCols(offset, Tr) = controls[i];
      Xref.middleCols(offset + 1, Tr) = paths[i].rightCols(Tr);
      offset += Tr;
    }
  }
};
...
RunStats runMppiLikeScenario(const Config &cfg, const std::string &solver_key,
  ...
  Solver solver(model);
  auto param = makeMppiParam(cfg, start, waypoints[1], cfg.global_T, N);
  ...
  for (int iter = 0; iter < cfg.maxiter; ++iter) {
    ...
    const int target_before_solve = target_index;
    ...
    const bool collided_after_move =
        collision_checker.getCollisionGrid(solver.x_init);
    if (collided_after_move) {
      stats.is_collision = true;
    } else {
      while (target_index < static_cast<int>(waypoints.size()) &&
        ...
        ++target_index;
        resetMppiWarmStart(solver, model.dim_u, cfg.global_T);
    ...
    appendStepTiming(stats, iter + 1, target_before_solve,
                     stats.reached_waypoints, stats.total_waypoints, solver);
    if (stats.is_collision) {
      break;
    }
    if (target_index >= static_cast<int>(waypoints.size())) {
      stats.success = true;
...
inline RunStats runBiScenario(const Config &cfg, const std::string &solver_key,
  ...
  WaypointBatchBiMPPI_GPU solver(model);
  auto param = makeBiParam(cfg, start, final_goal);
  ...
  for (int iter = 0; iter < cfg.maxiter; ++iter) {
    ...
    const int target_before_solve = target_index;
    std::vector<Eigen::VectorXd> anchors;
    anchors.reserve(static_cast<std::size_t>(waypoints.size() - target_index + 1));
    anchors.push_back(solver.x_init);
    for (int i = target_index; i < static_cast<int>(waypoints.size()); ++i) {
      anchors.push_back(waypoints[i]);
    }
...
int runMppiLikeExecutable(int argc, char **argv, const std::string &solver_key,
  ...
  try {
    ...
    std::ofstream timings(solver_dir / "timings.csv");
    writeTimingHeader(timings);
    ...
    for (int s = 0; s < cfg.scenarios; ++s) {
      ...
      writeTimingRows(timings, solver_key, s, stats);
...
inline int runBiExecutable(int argc, char **argv,
  ...
  try {
    ...
    const auto solver_dir = prepareSolverDir(cfg, solver_key);
    writeMetadata(cfg, solver_key, solver_label, solver_dir,
                  "multi_waypoint_bidirectional_connections",
                  "all_remaining_waypoint_segments_forward_backward_gpu_rollouts_then_global_guide");
