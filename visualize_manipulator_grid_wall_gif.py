#!/usr/bin/env python3
"""Render the grid-wall manipulator trajectory as a GIF."""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Create a GIF from build/manipulator_grid_wall_bicmppi_executed_x.csv"
    )
    parser.add_argument(
        "--build-dir",
        default="build",
        type=Path,
        help="Directory containing manipulator_grid_wall_bicmppi_*.csv",
    )
    parser.add_argument(
        "--out-dir",
        default=None,
        type=Path,
        help="GIF output directory. Defaults to <build-dir>/manipulator_grid_wall_gifs",
    )
    ...
    parser.add_argument(
        "--prefix",
        default="manipulator_grid_wall_bicmppi_",
...
def render_grid_wall(build_dir, out_dir, prefix, fps, max_frames):
    ...
    init_insert = metadata_point(metadata, "init_insert")
    init_retract = metadata_point(metadata, "init_retract")
    goal_retract = metadata_point(metadata, "goal_retract")
    goal_insert = metadata_point(metadata, "goal_insert")
    nominal_path = np.vstack([init_insert, init_retract, goal_retract, goal_insert])
    ...
    init_cell = metadata.get("init_cell", "?")
    ...
    def update(frame):
        ...
        step_text.set_text(
            f"Grid-wall BiC-MPPI | cell {init_cell}->{goal_cell} | step {frame}/{len(qs) - 1}"
        )
    ...
    gif_path = out_dir / "manipulator_grid_wall_bicmppi.gif"
...
def main():
    ...
    out_dir = (
        ...
        else build_dir / "manipulator_grid_wall_gifs"
    )
    gif_path = render_grid_wall(build_dir, out_dir, args.prefix, args.fps, args.max_frames)
