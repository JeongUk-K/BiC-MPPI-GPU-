#!/usr/bin/env python3
"""Render manipulator workspace-collision example trajectories as GIF files."""
...
START_Q = np.array([0.05, -1.15, 1.25, 0.0, 0.85, 0.0])
TARGET_Q = np.array([1.45, -0.80, 1.05, -0.20, 0.65, 0.25])
OBSTACLE_BOUNDS = ((-0.34, -0.20), (-0.52, -0.38), (0.28, 0.42))
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Create GIFs from build/manipulator_workspace_*_executed_x.csv"
    )
    parser.add_argument(
        "--build-dir",
        default="build",
        type=Path,
        help="Directory containing manipulator_workspace_*_executed_x.csv",
    )
    parser.add_argument(
        "--out-dir",
        default=None,
        type=Path,
        help="GIF output directory. Defaults to <build-dir>/manipulator_workspace_gifs",
    )
...
def obstacle_corners():
    (xmin, xmax), (ymin, ymax), (zmin, zmax) = OBSTACLE_BOUNDS
    return np.asarray(
        [
            [x, y, z]
            for x in (xmin, xmax)
...
def render_solver(build_dir, out_dir, solver_name, label, fps, max_frames):
    csv_path = build_dir / f"manipulator_workspace_{solver_name}_executed_x.csv"
    if not csv_path.exists():
        raise FileNotFoundError(f"{csv_path} not found. Run ./manipulaotr.sh first.")

    states = load_states(csv_path)
    qs = states[:, :6]
    pose_sequence = np.asarray([joint_positions(q) for q in qs])
    start_pose = joint_positions(START_Q)
    target_pose = joint_positions(TARGET_Q)
    ...
    def update(frame):
        ...
        step_text.set_text(f"{label} executed motion | step {frame}/{len(qs) - 1}")
    ...
    gif_path = out_dir / f"manipulator_workspace_{solver_name}.gif"
...
def main():
    ...
    out_dir = (
        ...
        else build_dir / "manipulator_workspace_gifs"
    )
